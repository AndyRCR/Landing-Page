<template>
  <div class="div-container" style="width: 100%; height: 100%">
...
export default {
  mounted() {
    ...
    const textMaterial = new THREE.MeshBasicMaterial({ color: "#08fdd8" });
    let loadCheck = false;
    ...
    const tick = () => {
      ...
      if (!mouseEnter) {
        ...
        if (!out) {
          group.rotation.y += cursor.mouseX * 0.02;
          group.rotation.x -= cursor.mouseY * 0.02;
        } else {
          group.rotation.y += cursor.mouseX * 0.005;
          group.rotation.x -= cursor.mouseY * 0.005;
        }
      }
